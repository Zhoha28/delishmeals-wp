	...
	GOYA = {
		...
		checkoutOrder: function() {
			...
			$(document.body).on( 'init_checkout updated_checkout payment_method_selected', function( event, data  ) {
				...
				$('form.checkout').arrive('.woocommerce-error, .woocommerce-info, woocommerce-message', function() {
					$( '.et-woocommerce-NoticeGroup' ).append($(this));
				});
				...
				self.shopAjax = $.ajax({
					...
					success: function(response) {
						...
						if ($newItems.length) {
							...
							// ITH Wishlist Premium reinit lightbox
							if (goya_theme_vars.settings.YITH_WCWL_Premium == true) {
							setTimeout( function(){
