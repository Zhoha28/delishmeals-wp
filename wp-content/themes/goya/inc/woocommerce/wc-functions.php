...
---------------------------------------------------------- */
	...
	function goya_progress_bar_content() {
		...
		if ( $subtotal < $threshold ) {
			$percent = floor(($subtotal / $threshold) * 100);
		}
		...
		<div class="free-shipping-progress-bar" data-progress="<?php echo esc_attr($percent); ?>">
			<div class="progress-bar-message">
				<?php
					if ( $percent == 100 ) {
						echo do_shortcode( wp_kses_post( $message_success ) );
					} else {
						echo do_shortcode( wp_kses_post( $message ) );
					}
				?>
			</div>
...
---------------------------------------------------------- */
	...
	if( get_theme_mod('product_sale_flash', 'pct') != 'disabled' || get_theme_mod('product_new_badge', true) == true ) {
		add_action( 'woocommerce_before_shop_loop_item_title', 'goya_product_badge', 9 );
	}
...
---------------------------------------------------------- */
	...
	function goya_sizing_guide_link() {
		...
		if ($size_guide && isset($size_page)) { 
			if ($mode == 'lightbox') { ?>
				<a href="#sizing-popup" rel="inline" class="sizing-guide-open sizing_guide open-lightbox" data-class="et-sizing-guide" data-button-inside="true"><?php echo get_the_title($size_page); ?></a>
			<?php } ?>
				<a href="<?php echo get_permalink($size_page); ?>" class="sizing-guide-open sizing_guide open-blank" data-class="et-sizing-guide" target="_blank"><?php echo get_the_title($size_page); ?></a>
			<?php
	}
	}
	...
	function goya_sizing_guide_content() {
		...
		$size_page = goya_sizing_guide();
		$mode = apply_filters('goya_size_guide_open_mode', 'lightbox');
		if ($size_guide && isset($size_page) && $mode == 'lightbox') { ?>
	...
	function goya_product_tabs() {
		...
		function goya_full_description_product() {
			...
			<div class="full_description">
				...
				<?php } else { ?>
					<?php 
					$heading = apply_filters( 'woocommerce_product_description_heading', __( 'Description', 'goya' ) );
					?>
