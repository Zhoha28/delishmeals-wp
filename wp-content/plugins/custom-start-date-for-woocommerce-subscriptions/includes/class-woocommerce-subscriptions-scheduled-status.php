...
class Scheduled_Woocommerce_Subscription_Status {
	...
	public function run() {
		 ...
		 */
		 ...
		 add_action( 'wscsd_start_date_changed', array( $this, 'wscsd_extends_is_activated' ), 100, 2 );
	...
	/**
	 * Registered new status by adding `Scheduled` to $registered_statuses array.
	 *
	...
	public function register_new_post_status( $registered_statuses ) {
		// translators: count
		$registered_statuses['wc-scheduled'] = _nx_noop( 'Scheduled <span class="count">( %s )</span>', 'Scheduled <span class="count">( %s )</span>', 'post status label including post count', 'custom-wcs-status-texts' );
		return $registered_statuses;
	}

	/**
	 * Add new status `Scheduled` to $subscription_statuses array.
	 * 
	...
	public function add_new_subscription_statuses( $subscription_statuses ) {
		$subscription_statuses['wc-scheduled'] = _x( 'Scheduled', 'Subscription status', 'custom-wcs-status-texts' );
	...
	public function extends_can_be_updated( $can_be_updated, $new_status, $subscription ) {
	 ...
	 //set the scheduled status on subscription with a custom start, which status was changed to active
	public function  wscsd_extends_is_activated ( $subscription_id, $date ) {
		$subscription = new WC_Subscription( $subscription_id );

		//$subscription->update_status( 'on-hold', __( 'Payment completed on order after subscription was cancelled.', 'woocommerce-subscriptions' ) );
		if ( metadata_exists( 'post', $subscription->get_id (), '_wscsd_delayed_start_date' ) ) {
			$scheduled_start = get_post_meta( $subscription->get_id (), '_wscsd_delayed_start_date', true );
			 //$next_payment = get_post_meta( $subscription->get_id (), '_schedule_next_payment', true );
			//$subscription->save();
			if ( ! empty( $scheduled_start ) ) {
				$today = gmdate( 'Y-m-d H:i:s' );

				if ( $today < gmdate( 'Y-m-d H:i:s', strtotime( $scheduled_start ) ) ) {
					//$subscription->set_status( 'wc-scheduled' );
					//$subscription->save();
					$subscription->update_status( 'scheduled' );
					//$subscription->update_dates( array( 'next_payment' => '2021-04-20 20:57:57' ) );
	 ...
	 * This code will also activate `reactivate` link in the list page for the subscription with `Scheduled` status
	 *
	 */
	public function enable_active_in_new_statuses( $can_be_updated, $subscription ) {
		if ( $subscription->payment_method_supports( 'subscription_reactivation' ) && $subscription->has_status( array( 'on-hold', 'scheduled' ) ) ) {
			$can_be_updated = true;
	 ...
	 *
	 * This function replaces the default code with the new one.
	 * This code will also activate `suspend` link in the list page for the subscription with `Scheduled` status
	 */
	public function enable_on_hold_in_new_statuses( $can_be_updated, $subscription ) {
		if ( $subscription->payment_method_supports( 'subscription_suspension' ) && $subscription->has_status( array( 'active', 'pending', 'scheduled' ) ) ) {
			$can_be_updated = true;
		} else {
			$can_be_updated = false;
		}
	...
	public function add_new_status_bulk_actions( $bulk_actions ) {
		$bulk_actions['scheduled'] = _x( 'Mark Scheduled', 'an action on a subscription', 'custom-wcs-status-texts' );
		return $bulk_actions;
	}
